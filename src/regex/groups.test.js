describe('regex groups', () => {
  ...
  test('finding a date pattern using matchAll and regex /(\\d{4})(\\d{2})(\\d{2})/g', () => {
    const matchAll = 'Deliver date: 20220603'.matchAll(/(\d{4})(\d{2})(\d{2})/g);
  ...
  test('finding a date pattern using matchAll and regex named groups /(?<year>\\d{4})(?<month>\\d{2})(?<day>\\d{2})/g', () => {
    const matchAll = 'Deliver date: 20220603'.matchAll(/(?<year>\d{4})(?<month>\d{2})(?<day>\d{2})/g);
